...
const f4 = (arr, elem) => {
  ...
  // реализуем возможность вводить значения в кавычках, чтобы получать индекс любого элемента ДАННОГО(a4) массива
  if (elem[0] == `'` || elem[0] == `"`) {
    let del = elem[0];
    elem = elem.replaceAll(del, "");
  } else elem = Number(elem);

  document.querySelector(".out-4").textContent = arr.indexOf(elem);
};
document.querySelector(".b-4").addEventListener("click", () => f4(a4));
